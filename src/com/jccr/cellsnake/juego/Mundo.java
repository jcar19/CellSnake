...
public class Mundo {
	...
	public JollyRoger jolliRoger;
	public Botin botin;
	public boolean finaJuego = false;
	...
	private void colocarBotin() {
		...
		int len = jolliRoger.partes.size();
		for(int i = 0; i < len; i++){
			Tripulacion parte = jolliRoger.partes.get(i);
	...
	public void update(float deltaTime){
		...
		while(tiempoTick > tick){
			tiempoTick -= tick;
			jolliRoger.avance();
			if(jolliRoger.comprobarChoque()){
				finaJuego = true;
				return;
			}
			
			Tripulacion head = jolliRoger.partes.get(0);
			if(head.x == botin.x && head.y == botin.y){
				puntuacion += INCREMENTO_PUNTUACION;
				jolliRoger.expulsion();
				if(jolliRoger.partes.size() == MUNDO_ANCHO * MUNDO_ALTO){
					finaJuego = true;
					return;
				} else{
					colocarBotin();
